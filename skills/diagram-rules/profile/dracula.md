...
name: Dracula
slug: dracula
source-url: https://draculatheme.com
created: 2026-08-19
updated: 2026-09-04
notes: Dark skin for docs/Architect diagrams viewed in VS Code
-->
...
This skin is Dracula — `#282a36` paper, `#f8f8f2` ink, `#ff79c6` pink accent, `#8be9fd` cyan links. Diagrams are authored in the dark variant. It's designed to look good out of the box; swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#fffbeb` (alucard-bg) | `#282a36` (dracula-bg) |
| `paper-2` | Diagram container bg, secondary fill | `#efe9d5` | `#44475a` (current-line) |
| `ink` | Primary text, primary stroke | `#1f1f1f` (alucard-fg) | `#f8f8f2` (dracula-fg) |
| `muted` | Secondary text, default arrow stroke | `#6c664b` (alucard-comment) | `#bfc7e6` (lightened comment) |
| `soft` | Sublabels, boundary labels | `#8a8468` | `#8f9ac4` |
| `rule` | Hairline borders | `rgba(31,31,31,0.12)` | `rgba(248,248,242,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#cfcfde` | `rgba(248,248,242,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#a3144d` (alucard-pink) | `#ff79c6` (dracula-pink) |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(163,20,77,0.08)` | `rgba(255,121,198,0.12)` |
| `link` | HTTP/API calls, external arrows | `#036a96` (alucard-cyan) | `#8be9fd` (dracula-cyan) |

> **Brand palette source:** this skin maps to the official Dracula palette — `bg #282a36`, `current-line #44475a`, `fg #f8f8f2`, `comment #6272a4`, `cyan #8be9fd`, `pink #ff79c6`, `purple #bd93f9`. The light column is Alucard, Dracula's official light counterpart. `muted` is a lightened comment (the official `#6272a4` fails contrast for 8px body text on `#282a36`); `soft` and `rule` are derived to cover roles the palette doesn't name directly. Hand-drawn diagrams in this skin draw connector strokes at `#7d88b8` — deliberately quieter than `muted` so arrows sit behind sublabels rather than competing with them.
